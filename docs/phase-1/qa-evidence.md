...
- **F1 Call intelligence**: one structured Claude analysis per connected call (7 artifact kinds, each with confidence + transcript-turn evidence, model + prompt version provenance) persisted to `call_artifacts` with an append-only supersede chain — a human edit supersedes; AI can NEVER supersede a human row. Disposition policy (`ai.dispositionPolicy`): auto-apply only at ≥0.8 confidence with a transcript and never for `do_not_call`; everything else lands in the **Needs Review** lane on /callbacks (accept / change / dismiss, resolution audited) with a nav badge + realtime `review.created` events. Transcript timestamps now survive to the UI: click-a-line **seek-to-line** in the call detail modal (legacy rows degrade honestly without the affordance). AI vs human content is visually distinguished everywhere (AiSourceBadge + provenance lines).
- **F2 Report center + leaderboard v2**: every KPI/funnel stage/outcome row/rep row **drills down** to the matching `/leads?f=` set (archive has no filter grammar — honestly not linked); metric-definition tooltips from the glossary on dashboard + reports; "Data as of" stamps with org timezone + explicit range dates; previous-period comparison with deltas; saved report views (≤12, org settings). Leaderboard: **configurable points** (admin scoring section), **calendar-true** day/week/month in org tz (the rolling-window mislabels are gone), per-entry point breakdowns that sum exactly, streaks + personal best, deterministic ties, live updates via `leaderboard.delta`. `appointmentKept` excluded honestly (no kept/completed signal exists in the data model yet).
